...
Trains a Pix2Pix conditional GAN to translate ground-based HSC (Hyper
Suprime-Cam) images to space-based HST (Hubble Space Telescope) quality
using paired FITS image cutouts.

The training loop alternates between generator and discriminator updates.
The generator is updated every step; the discriminator is updated at a
configurable frequency (``disc_update_freq``).  All hyperparameters are
read from an INI configuration file (see ``neo/configs/example.ini``).

Metrics and sample images are logged to Comet ML for experiment tracking.
...
def main() -> None:
    """Parse config, build dataloaders and model, and run the training loop."""
    ...
    # Data paths (paired HST/HSC FITS directories for train and val splits).
    ...
    hst_dim = int(config["HST_DIM"]["hst_dim"])  # 768
    hsc_dim = int(config["HSC_DIM"]["hsc_dim"])  # 128
    ...
    # Loss component weights (see ``neo/pix2pix.py`` for details).
    ...
    # How often to update the discriminator (every N generator steps).
    disc_update_freq = int(config["DISC_UPDATE_FREQ"]["disc_update_freq"])

    # Optional pretrained model filenames (empty string = train from scratch).
    ...
    # Per-layer VGG loss weights: [relu1_1, relu2_1, relu3_1, relu4_1, relu5_1].
    vgg_loss_weights = eval(config["VGG_LOSS_WEIGHTS"]["vgg_loss_weights"])

    # ------------------------------------------------------------------
    # 2. Initialize Comet ML experiment tracking
    # ------------------------------------------------------------------
    ...
    # Descriptive model name used in checkpoint filenames.
    model_name = (
    ...
    # ------------------------------------------------------------------
    # Training dataloader: shuffled, with custom collate_fn that filters
    # corrupted/invalid samples.
    dataloader_train = torch.utils.data.DataLoader(
        SR_HST_HSC_Dataset(
            hst_path=hst_path_train,
            hsc_path=hsc_path_train,
    ...
    # Validation dataloader: converted to an iterator so we can call next()
    # to draw a single validation batch at each display step.
    dataloader_val = torch.utils.data.DataLoader(
        SR_HST_HSC_Dataset(
            hst_path=hst_path_val,
            hsc_path=hsc_path_val,
            hr_size=[hst_dim, hst_dim],
            lr_size=[hsc_dim, hsc_dim],
            data_aug=data_aug,
            experiment=experiment,
    ...
    # 4. Initialize model
    # ------------------------------------------------------------------
    # The ``input_size`` of 600 is the center-cropped spatial dimension used
    # inside the loss computation (see ``Pix2Pix._gen_step``).
    pix2pix = Pix2Pix(
        in_channels=1,
        out_channels=1,
        input_size=600,
    ...
    while cur_step < total_steps:
        for hr_real, lr, hsc_hr, seg_map_real in tqdm(dataloader_train, position=0):
            ...
            losses = pix2pix.training_step(
                hr_real, lr, hsc_hr, seg_map_real, "generator"
            )
            ...
            if cur_step % disc_update_freq == 0:
                disc_losses = pix2pix.training_step(
                    hr_real, lr, hsc_hr, seg_map_real, "discriminator"
                )
                disc_loss = disc_losses[0].item()
                fake_disc_logits = disc_losses[1]
                real_disc_logits = disc_losses[2]

            # --- Validation and logging (every ``display_step`` steps) ---
            if cur_step % display_step == 0 and cur_step > 0:
                hr_real_val, lr_val, hsc_hr_val, seg_map_real_val = next(
                    dataloader_val
                )
                ...
                val_losses = pix2pix.validation_step(
                    hr_real_val, lr_val, hsc_hr_val, seg_map_real_val, "generator"
                )
                ...
                disc_val_losses = pix2pix.validation_step(
                    hr_real_val, lr_val, hsc_hr_val, seg_map_real_val,
                    "discriminator",
                )
                ...
                print(
                    ...
                    f"Discriminator loss: {disc_val_loss:.5f}"
                )

                # Extract single images (first in batch) for visualization.
                ...
                # Compute pixel-wise difference map for residual visualization.
                img_diff = (
                    CenterCrop(600)(fake_val - hr_val).cpu().detach().numpy()
                )
                vmax = np.abs(img_diff).max()

                # Log visualization figures to Comet ML.
                log_figure(
                    CenterCrop(100)(lr_val_img).detach().numpy(),
                    "100x100 Conditioned Val Image (HSC)",
                    experiment,
                )
                log_figure(
                    CenterCrop(600)(fake_val).detach().numpy(),
                    "600x600 Generated Val Image (SR)",
                    experiment,
                )
                log_figure(
                    CenterCrop(600)(hr_val).detach().numpy(),
                    "600x600 Real Val Image (HST)",
                    experiment,
                )
                log_figure(
                    real_disc_val_map.detach().numpy(),
                    "Real Disc Val Logits",
                    experiment,
                )
                log_figure(
                    fake_disc_val_map.detach().numpy(),
                    "Fake Disc Val Logits",
                    experiment,
                )
                log_figure(
                    img_diff,
                ...
                experiment.log_metric(
                    "L1 Val Scattering Loss", scattering_val_loss
                )
                experiment.log_metric(
                    "L1 Val Segmap Reconstruction Loss", segmap_val_loss
                )
                experiment.log_metric(
                    "L1 Val Segmap/L1 Recon Ratio",
                    segmap_val_loss / recon_val_loss,
                )
            ...
            if cur_step % save_steps == 0 and cur_step > 0:
                torch.save(
                    pix2pix.gen,
                    f"models/gen_pix2pixsr_{model_name}_checkpoint_{cur_step}.pt",
                )
                torch.save(
                    pix2pix.patch_gan,
                    f"models/patchgan_pix2pixsr_{model_name}_checkpoint_{cur_step}.pt",
                )
